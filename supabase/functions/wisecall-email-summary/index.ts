...
serve(async (req) => {
  ...
  const willEmail = !emailAlreadySent && to.length > 0 && Boolean(resendKey);
  ...
  if (emailAlreadySent) {
    ...
    const emailInput = {
      ...
      callerName: insights?.callerName || null,
    ...
    const subjectWho = insights?.callerName || callerId;
    const subjectPrefix = insights?.urgency === "emergency" ? "EMERGENCY · " : "";
    const subject = actionItems.length
      ? `${subjectPrefix}Follow-up needed · ${subjectWho} · ${businessName}`
      : `${subjectPrefix}Message from ${subjectWho} · ${businessName}`;
