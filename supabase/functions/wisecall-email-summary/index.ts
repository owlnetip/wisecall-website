...
// metadata. Staff mobiles come from metadata.staff_alert_sms, else notify
// routing contacts, gated by wisecall_profiles.sms_enabled.
...
serve(async (req) => {
  ...
  const identity = resolveCallerIdentity({
    callerId,
    collected,
    analysis: callLog?.ai_analysis_json,
    summary,
    transcript,
  });
