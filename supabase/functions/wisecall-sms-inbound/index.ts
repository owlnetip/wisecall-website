...
import { routeConfirmedSalesforceReply } from "../_shared/salesforce-sms-inbound-router.ts";
...
async function handleInbound(params: Record<string, unknown>): Promise<void> {
  ...
  const salesforceRouted = await routeConfirmedSalesforceReply({
    ...
    findBinding: async (digits) => {
      const { data, error } = await supabase.from("wisecall_salesforce_sms_bindings")
    ...
    recordFailure: async (binding, digits, reason) => {
      const { error } = await supabase.from("wisecall_salesforce_sms_messages").insert({
        profile_id: profile.id, binding_id: binding.id, direction: "inbound",
        phone_digits: digits, body: inbound.text, status: "route_failed",
        salesforce_record_id: binding.salesforce_record_id, provider: "salesforce",
        provider_message_id: inbound.messageId || null, detail: { error: reason },
      });
      if (error) throw new Error("Salesforce reply failure could not be stored");
    },
  });
  if (salesforceRouted) return;
