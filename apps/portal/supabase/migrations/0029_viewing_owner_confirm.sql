-- Purplebricks / Strike-style viewing booking with owner confirmation.
--
-- Flow:
--   1. Viewer requests a slot (phone / WhatsApp / SMS / manual)
--   2. Agent availability is checked when a calendar connector exists
--   3. Owner is asked via WhatsApp (prefer) or SMS to approve the slot
--   4. On YES → confirm to viewer (+ agent note); on NO / CHANGE → branch
--   5. Day-before "still ok?" SMS + day-of reminders to both parties
--
-- Service-role only (no anon/authenticated grants). Matches other WiseCall
-- operational tables after the 0025 RLS tighten.

-- Properties an agency manages (owner contact details live here).
create table if not exists wisecall_properties (
  ...
  owner_phone     text,
  owner_email     text,
  -- auto = WhatsApp if the agent has WA, else SMS
...
-- Viewing request state machine (owner-confirm loop).
create table if not exists wisecall_viewing_requests (
  ...
  -- Snapshot of the property/owner at request time (survives property edits).
  ...
  -- requested → pending_owner → confirmed | declined | change_requested
...
-- Audit trail for every outbound/inbound message in the loop.
