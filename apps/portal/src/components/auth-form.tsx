...
function Owl({ eyeOffset, passwordFocused, blinking }: OwlProps) {
  return (
    ...
    >
      ...
      <g style={{ transformOrigin: "28px 96px", transform: passwordFocused ? "translate(20px, -30px) rotate(-104deg)" : "rotate(0deg)", transition: "transform 0.5s cubic-bezier(0.34,1.56,0.64,1)" }}>
      ...
      <g style={{ transformOrigin: "92px 96px", transform: passwordFocused ? "translate(-20px, -30px) rotate(104deg)" : "rotate(0deg)", transition: "transform 0.5s cubic-bezier(0.34,1.56,0.64,1)" }}>
...
export function AuthForm({
  ...
  return (
    <div
      className="relative flex min-h-screen w-full items-center justify-center overflow-x-hidden overflow-y-auto py-8"
    ...
    >
      ...
      <div className="relative z-10 mx-4 w-full max-w-sm">
        <div ref={owlContainerRef} className="mb-1 flex justify-center">
        ...
        >
          <div className="mb-7 text-center">
            <div className="mb-1.5 flex items-center justify-center gap-0">
              <span className="text-2xl font-bold tracking-tight text-white">Wise</span>
              <span className="text-2xl font-bold tracking-tight" style={{ color: "#7de8eb" }}>
                Call
              </span>
            </div>
          ...
          <form action={formAction} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                {!isSignup ? (
                  <button
                    ...
                    onClick={() => startReset(async () => setResetState(await resetPassword(email)))}
              ...
              />
              {passwordFocused ? (
                <p className="mt-1 text-xs" style={{ color: "rgba(125,232,235,0.6)" }}>
                  🦉 Not peeking, promise.
                </p>
              ) : null}
            </div>
            ...
            {state.error ? (
              <p className="rounded-lg px-3 py-2 text-xs font-medium" style={{ background: "rgba(255,99,99,0.12)", color: "#ff9b9b", border: "1px solid rgba(255,99,99,0.25)" }}>
                {state.error}
              </p>
            ) : null}
            {state.message ? (
              <p className="rounded-lg px-3 py-2 text-xs font-medium" style={{ background: "rgba(125,232,235,0.12)", color: "#7de8eb", border: "1px solid rgba(125,232,235,0.25)" }}>
                {state.message}
              </p>
            ) : null}
            ...
            <button
              ...
              style={{ background: "#7de8eb", color: "#172929", boxShadow: "0 2px 12px rgba(125,232,235,0.3)" }}
            >
              {isPending
                ? isSignup
                  ? "Creating account…"
