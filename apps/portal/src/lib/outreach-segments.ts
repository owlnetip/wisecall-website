/** Shared segment / vertical helpers for dental + property outreach CRM. */

export type OutreachVertical = "dental" | "property";
...
export function defaultSegmentForVertical(vertical: OutreachVertical): string {
  return vertical === "property" ? "property_unknown" : "dentally_active";
}

export function emailableSegmentForVertical(vertical: OutreachVertical): string {
  return vertical === "property" ? "property_ready" : "dentally_active";
}
...
export function verticalForSegment(segment: string): OutreachVertical {
  return segment.startsWith("property_") ? "property" : "dental";
}
...
export function segmentErrorMessage(segment: string): string {
  ...
  if (segment === "corporate_hold" || segment === "property_corporate_hold") {
