 ...
 *
 * Dark WiseCall branded template (summary + labelled conversation), with the
 * caller name at the top when captured. Next actions match the portal inbox.
 */
...
const INTERNAL_TRANSCRIPT_LINE =
  /^(?:\[system\]|system\s*:|\[(?:thinking|think|internal|debug)\]|<(?:thinking|think)>)/i;
const INTERNAL_TRANSCRIPT_TOKEN = /\bSLOW_(?:THINK|SPEAK)_REQUEST\b/i;
const SPEAKER_LINE =
...
/** Voice-runtime notes (think/speak watchdogs), not something the caller said. */
export function isInternalTranscriptLine(line: string): boolean {
  const trimmed = line.trim();
  if (!trimmed) return false;
  return INTERNAL_TRANSCRIPT_LINE.test(trimmed) || INTERNAL_TRANSCRIPT_TOKEN.test(trimmed);
}

export function customerFacingTranscript(transcript: string): string {
...
function isToolLogLine(text: string): boolean {
  ...
  if (isInternalTranscriptLine(trimmed)) return true;
...
function displayAgentName(agentName?: string): string {
  ...
  // Desk labels such as "Charles Garth Voice Desk" are not a person's name.
  if (/\b(voice|desk|receptionist|assistant|surveyors?)\b/i.test(name)) return "WiseCall";
...
function transcriptHtml(input: PostCallEmailInput): string {
  const transcript = customerFacingTranscript(input.transcript);
  ...
  const lines = turns.length
    ? turns
        ...
        .join("")
    : `<p style="margin:0;line-height:1.55;color:#ffffff;">${nl2br(customerFacingTranscript(transcript))}</p>`;
